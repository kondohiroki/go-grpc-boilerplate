...
openssl req -passin pass:${PASSWORD} -new -x509 -sha256 -days 3650 -key ca.key -out ca.crt -subj "/CN=ca"
...
openssl x509 -req -extfile <(printf "subjectAltName=DNS:${SERVER_CN}") -passin pass:${PASSWORD} -sha256 -days 3650 -in server.csr -CA ca.crt -CAkey ca.key -set_serial 01 -out server.crt 
